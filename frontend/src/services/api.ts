...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response?.status === 429) {
      const message = error.response?.data?.detail || 'El sistema esta ocupado. Intenta mas tarde.'
      error.userMessage = message
    }
    // timeout
    else if (error.code === 'ECONNABORTED' || error.message?.includes('timeout')) {
      error.userMessage = 'La solicitud excedio el tiempo de espera. Revisa el estado del proyecto.'
    }
    // network/backend unreachable
    else if (error.code === 'NETWORK_ERROR' || error.code === 'ERR_NETWORK' || !error.response) {
      error.userMessage = `No hay conexion con el backend (${API_BASE_URL}). Verifica que AutoClip backend este levantado en el puerto 8000.`
    }
    // server error
    else if (error.response?.status >= 500) {
      error.userMessage = 'Error interno del servidor. Intenta nuevamente.'
    }
...
const normalizeProject = (project: any): Project => {
  const normalizedProcessingConfig =
    project?.processing_config ?? project?.settings ?? {}

  return {
    ...project,
    processing_config: normalizedProcessingConfig,
  } as Project
}
...
export const projectApi = {
  ...
  uploadFiles: async (data: UploadFilesRequest): Promise<Project> => {
    ...
    const project = await api.post('/projects/upload', formData, {
      headers: {
