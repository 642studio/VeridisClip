...
const HomePage: React.FC = () => {
  ...
  //       
  //     default:
  //       console.log('忽略Desconocido类型的WebSocket消息:', (message as any).type)
  //   }
  // }
  ...
  //     console.log('同步订阅项目Canal:', desiredChannels)
  ...
  const handleRetryProject = async (projectId: string) => {
    try {
      ...
      message.success('Reintento de procesamiento iniciado')
      
      await loadProjects()
    } catch (error) {
      message.error('Error al reintentar. Intenta de nuevo mas tarde')
  ...
  const handleStartProcessing = async (projectId: string) => {
    try {
      ...
      message.success('El procesamiento ha iniciado. Revisa el progreso en unos segundos')
      // 立即刷新项目列表以显示最新状态
      setTimeout(async () => {
        try {
          await refreshNow()
      ...
      const errorMessage = (error as { userMessage?: string })?.userMessage || 'Error al iniciar el procesamiento'
      ...
      if ((error as { code?: string; message?: string })?.code === 'ECONNABORTED' || (error as { code?: string; message?: string })?.message?.includes('timeout')) {
        message.info('La solicitud expiro, pero el proyecto podria estar procesandose. Revisa el estado', 5)
        // 延迟刷新项目列表
        setTimeout(async () => {
          try {
            await refreshNow()
  ...
  const handleProjectCardClick = (project: Project) => {
    // Importando...态的项目不能点击进入详情页
    if (project.status === 'pending') {
      message.warning('El proyecto sigue importandose. Revisa el detalle en un momento')
      return
    }
              ...
              <div>
                ...
                {activeTab === 'upload' && (
                  <FileUpload onUploadSuccess={async (projectId: string) => {
                    // 处理完成后刷新项目列表
                    await loadProjects()
                    message.success('Proyecto creado. Procesando en segundo plano.')
                  }} />
                )}
              </div>
            </div>
            ...
            <div style={{ 
              ...
              <div style={{ 
                ...
                <Select
                  ...
                  allowClear
                >
                  <Option value="all" style={{ color: '#ffffff' }}>Todos los estados</Option>
                  <Option value="completed" style={{ color: '#52c41a' }}>Completado</Option>
                  <Option value="processing" style={{ color: '#1890ff' }}>En proceso</Option>
                  <Option value="error" style={{ color: '#ff4d4f' }}>Fallido</Option>
                </Select>
              </div>
            </div>
                 ...
                 <div style={{
                   ...
                   <Empty
                     ...
                     description={
                       <div>
                         <Text type="secondary">
                           {projects.length === 0 ? 'Aun no hay proyectos. Usa el modulo de importacion para crear el primero' : 'No se encontraron proyectos con ese filtro'}
                         </Text>
                       </div>
                     }
                   />
