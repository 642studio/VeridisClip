...
const FileUpload: React.FC<FileUploadProps> = ({ onUploadSuccess }) => {
  ...
  const onDrop = (acceptedFiles: File[]) => {
    ...
    acceptedFiles.forEach(file => {
      ...
      if (['mp4', 'avi', 'mov', 'mkv', 'webm'].includes(extension || '')) {
        newFiles.video = file
        // 自动设置Nombre del proyecto为视频文件名（去掉扩展名）
        // 每次选择新视频文件时都更新Nombre del proyecto
  ...
  const handleUpload = async () => {
    if (!files.video) {
      message.error('Selecciona un archivo de video')
      return
    }

    if (!projectName.trim()) {
      message.error('Ingresa un nombre de proyecto')
      return
    }
    ...
    try {
      ...
      console.log('Iniciando carga de archivos:', {
        ...
        srt_file: files.srt?.name || '(se generara con reconocimiento de voz)',
      ...
      console.log('Carga exitosa, proyecto:', newProject)
      
      clearInterval(progressInterval)
      setUploadProgress(100)
      
      addProject(newProject)
      message.success('Proyecto creado correctamente. Procesando en segundo plano...')
      ...
      console.error('Error de carga, detalle:', error)
      
      let errorMessage = 'Error de carga. Intenta nuevamente'
      let errorType = 'error'
      
      // 根据错误类型提供更友好的错误信息
      if (error.response?.status === 413) {
        errorMessage = 'El archivo es demasiado grande. Selecciona uno mas pequeno'
        errorType = 'warning'
      } else if (error.response?.status === 415) {
        errorMessage = 'Formato no soportado. Usa MP4, AVI, MOV, MKV o WEBM'
        errorType = 'warning'
      } else if (error.response?.status === 400) {
        if (error.response?.data?.detail) {
          errorMessage = error.response.data.detail
        } else {
          errorMessage = 'El archivo tiene un formato o contenido invalido. Revisa e intenta de nuevo'
        }
      } else if (error.response?.status === 500) {
        errorMessage = 'Error del servidor al procesar el archivo. Intenta mas tarde'
      } else if (error.code === 'ECONNABORTED') {
        errorMessage = 'La carga expiro. Revisa tu conexion e intenta de nuevo'
      ...
      if (error.code === 'ECONNABORTED' || error.response?.status >= 500) {
        message.info('Si el problema continua, revisa la red o contacta soporte', 5)
      }
    } finally {
      setUploading(false)
    }
  ...
  return (
    <div style={{
      ...
      >
        ...
        <div>
          <Text strong style={{ 
            ...
            {isDragActive ? 'Suelta para importar archivo' : 'Haz clic o arrastra archivos aqui'}
          </Text>
          <Text style={{ color: '#cccccc', fontSize: '14px', lineHeight: '1.5' }}>
            Formatos compatibles: MP4, AVI, MOV, MKV y WebM. <Text style={{ color: '#52c41a', fontWeight: 600 }}>Puedes importar un archivo de subtitulos (.srt) o generarlos con IA</Text>
          </Text>
        </div>
      </div>

      {/* Nombre del proyecto输入 - 只有在选择文件后才显示 */}
      {files.video && (
        <div style={{ marginBottom: '16px' }}>
          ...
          <Input
            ...
            placeholder="Ingresa un nombre para identificar tu proyecto"
            style={{ 
              height: '40px',
              borderRadius: '12px',
              fontSize: '14px',
      ...
      {/* Categoria de video选择 - 只有在选择文件后才显示 */}
      {files.video && (
        <div style={{ marginBottom: '16px' }}>
          <Text strong style={{ color: '#ffffff', fontSize: '14px', marginBottom: '8px', display: 'block' }}>
            Categoria de video
          ...
          {files.video && !files.srt && (
            <div style={{
              ...
              <Text style={{ color: '#52c41a', fontSize: '14px', fontWeight: 500 }}>
                Se generaran subtitulos automaticamente con reconocimiento de voz IA
              </Text>
            </div>
          )}
        </div>
      ...
      {uploading && (
        <div style={{ 
          ...
          <div style={{ marginBottom: '12px' }}>
            <Text style={{ color: '#ffffff', fontWeight: 600, fontSize: '14px' }}>Progreso de importacion</Text>
          ...
          <Text style={{ color: '#cccccc', fontSize: '13px', marginTop: '8px', display: 'block', textAlign: 'center' }}>
            Importando archivos, espera por favor...
          </Text>
        </div>
      )}
      ...
      {files.video && (
        <div style={{ textAlign: 'center', marginTop: '8px' }}>
          ...
          >
            {uploading ? 'Importando...' : 'Iniciar importacion y procesamiento'}
          </Button>
        </div>
      )}
    </div>
