...
from ..utils.llm_debug import is_llm_debug_enabled, write_llm_debug_event
...
class OutlineExtractor:
        ...
        for i, chunk_file in enumerate(chunk_files):
            ...
            try:
                ...
                if response:
                    ...
                    logger.info(f"第{i+1}块解析出{len(parsed_outlines)}个话题")
    ...
    def _parse_outline_response(self, response: str, chunk_index: int) -> List[Dict]:
        ...
        for line in lines:
            ...
            elif (line.startswith('-') or line.startswith('•') or line.startswith('*')) and current_outline:
        ...
        # 回退: 若Markdown解析失败，尝试JSON解析
        if not outlines:
            try:
                parsed = self.llm_client.parse_json_response(response)
                if isinstance(parsed, dict):
                    parsed = parsed.get("outline", parsed.get("outlines", []))
                if isinstance(parsed, list):
                    for item in parsed:
                        if not isinstance(item, dict):
                            continue
                        title = str(item.get("title") or item.get("outline") or "").strip()
                        if not title:
                            continue
                ...
                logger.warning(f"块 {chunk_index} JSON回退解析失败: {e}")
