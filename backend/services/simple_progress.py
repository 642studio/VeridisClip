...
def _get_redis_client():
    """Get or reconnect Redis client lazily."""
    global r
    if r is not None:
        return r
    try:
...
def emit_progress(project_id: str, stage: str, message: str = "", subpercent: Optional[float] = None):
    ...
    try:
        # 1) 持久化最新快照（给轮询/刷新用）
        client.hset(f"progress:project:{project_id}", mapping={
            "stage": stage, 
            "percent": str(percent), 
            "message": message, 
            "ts": str(payload["ts"])
        ...
        client.publish(f"progress:project:{project_id}", json.dumps(payload))
...
def get_progress_snapshot(project_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        h = client.hgetall(f"progress:project:{project_id}")
        if not h:
            return None
...
def clear_progress(project_id: str):
    ...
    try:
        client.delete(f"progress:project:{project_id}")
