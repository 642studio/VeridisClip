...
class LLMClient:
    ...
    def _with_language_guard(self, prompt: str) -> str:
        """
        Enforce Spanish output for user-facing text while preserving requested schemas.
        """
        language_guard = (
            "\n\n"
            "=== REGLAS OBLIGATORIAS DE IDIOMA ===\n"
            "1) Responde SIEMPRE en espanol neutro.\n"
            "2) No uses chino ni transliteraciones en chino.\n"
            "3) Si la salida debe ser JSON, conserva exactamente la estructura y claves solicitadas.\n"
            "4) Traduce al espanol todos los valores textuales de la salida.\n"
            "5) No agregues explicaciones fuera del formato solicitado.\n"
        )
        return f"{prompt}{language_guard}"
    ...
    def call(self, prompt: str, input_data: Any = None) -> str:
        ...
        """
        try:
            return self.llm_manager.call(self._with_language_guard(prompt), input_data)
    ...
    def call_with_retry(self, prompt: str, input_data: Any = None, max_retries: int = 3) -> str:
        ...
        try:
            return self.llm_manager.call_with_retry(
                self._with_language_guard(prompt),
                input_data,
                max_retries
            )
        except Exception as e:
