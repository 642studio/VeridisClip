...
# Avoid repeated expensive install attempts on every task.
BCUT_AUTO_INSTALL_ATTEMPTED = False
...
def _ensure_bcut_asr_available():
    ...
    global BCUT_AUTO_INSTALL_ATTEMPTED
    
    if BCUT_ASR_AVAILABLE:
        return True

    # bcut-asr currently requires Python >=3.10
    if sys.version_info < (3, 10):
        logger.warning("当前Python版本低于3.10，跳过bcut-asr自动安装")
        return False

    if BCUT_AUTO_INSTALL_ATTEMPTED:
        logger.info("bcut-asr已尝试自动安装，跳过重复安装")
        return False

    BCUT_AUTO_INSTALL_ATTEMPTED = True
