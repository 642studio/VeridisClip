...
def _detect_video_platform(url: str) -> str:
    ...
    if "youtube.com" in normalized or "youtu.be" in normalized:
        return "youtube"
    if "facebook.com" in normalized or "fb.watch" in normalized:
        return "facebook"
    return "unknown"
...
async def _extract_youtube_info(url: str, browser: Optional[str] = None) -> dict:
    """Extract YouTube info and gracefully fallback if browser cookies are blocked."""
    ydl_opts = {
        'quiet': True,
        'no_warnings': True,
    }
    ...
    try:
        return await loop.run_in_executor(None, extract_info_sync, url, ydl_opts)
    except Exception as e:
        err = str(e)
        if browser and ("Operation not permitted" in err or "Cookies.binarycookies" in err):
            logger.warning("No se pudieron leer cookies del navegador, continuando sin cookies")
            fallback_opts = {'quiet': True, 'no_warnings': True}
            return await loop.run_in_executor(None, extract_info_sync, url, fallback_opts)
        raise


def _base_youtube_ydl_opts(
    download_dir: Path,
    ...
    """Build stable yt-dlp options to reduce frequent YouTube 403 failures."""
    subtitle_langs = subtitle_langs or ['en', 'es', 'zh-Hans', 'zh', 'en-US', 'auto']
    opts = {
...
async def _download_youtube_with_fallback(url: str, download_dir: Path, browser: Optional[str] = None):
    """Try download without cookies first, then common browser cookies as fallback."""
    import asyncio

    platform = _detect_video_platform(url)
    attempts = [browser.lower(), None] if browser else [None]
    ...
    loop = asyncio.get_event_loop()
    for browser_name in attempts:
        ydl_opts = _base_youtube_ydl_opts(download_dir, platform=platform, include_subtitles=False)
        if browser_name:
        ...
        try:
            ...
            last_error = e
            logger.warning(f"YouTube descarga fallida con cookies={browser_name or 'none'}: {e}")

    error_text = str(last_error) if last_error else "Error desconocido"
    if "403" in error_text:
        raise Exception("YouTube bloqueó la descarga (HTTP 403). Configura cookies de navegador en Configuración y vuelve a intentar.")
    raise Exception(error_text)


async def _mark_project_download_failed(project_id: str, message: str):
    """Persist project failure so UI does not stay in pending state."""
    try:
        ...
        try:
            ...
            project.processing_config["download_status"] = "failed"
            project.processing_config["download_progress"] = 0.0
            project.processing_config["error_message"] = message
            db.commit()
        finally:
            db.close()
    except Exception as e:
        logger.error(f"No se pudo marcar proyecto {project_id} como fallido: {e}")
...
async def parse_youtube_video(
    ...
    """解析视频信息（YouTube/Facebook）"""
    try:
        logger.info(f"开始解析视频: {url}")
        platform = _detect_video_platform(url)
        if platform not in {"youtube", "facebook"}:
            raise HTTPException(status_code=400, detail="Enlace invalido. Usa un enlace de YouTube o Facebook.")
        
        # 获取视频信息（若浏览器cookies权限受限，自动回退到无cookies）
        info_dict = await _extract_youtube_info(url, browser)
        
        logger.info(f"Video解析成功({platform}): {info_dict.get('title', 'Unknown')}")
...
async def create_youtube_download_task(request: YouTubeDownloadRequest):
    """创建视频下载任务（YouTube/Facebook） - 立即创建项目"""
    try:
        logger.info(f"创建下载任务: {request.url}")
        platform = _detect_video_platform(request.url)
        if platform not in {"youtube", "facebook"}:
            raise HTTPException(status_code=400, detail="Enlace invalido. Usa un enlace de YouTube o Facebook.")
        
        # 先获取视频信息以获取缩略图（cookies失败时自动回退）
        video_info = await _extract_youtube_info(request.url, request.browser)
        ...
        try:
            ...
            if thumbnail_url:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.info(f"Miniatura obtenida ({platform}): {video_info.get('title', 'Unknown')}")
                    else:
                        logger.warning(f"Fallo al descargar miniatura: {response.status_code}")
                except Exception as e:
            ...
            project_data = ProjectCreate(
                ...
                description=f"Descargado desde {platform}: {video_info.get('title', 'Unknown')}",
...
async def process_youtube_download_task(task_id: str, request: YouTubeDownloadRequest, project_id: str):
    ...
    try:
        ...
        await _download_youtube_with_fallback(request.url, download_dir, request.browser)
        
        # 查找下载的文件（仅当前任务目录，避免跨任务串档）
        video_files = sorted(
        ...
        try:
            ...
            # 更新项目信息
            platform = _detect_video_platform(request.url)
            project.description = f"Descargado desde {platform}: {request.project_name}"
            # 注意：不要在这里设置video_path，等文件移动完成后再设置
            
            # 更新项目设置
            cfg = dict(project.processing_config or {})
            existing_info = dict(cfg.get("youtube_info", {}))
            cfg.update({
                "youtube_info": {
                    **existing_info,
                    "platform": platform,
                    "title": existing_info.get("title", request.project_name),
                    "uploader": existing_info.get("uploader", ""),
                    "duration": existing_info.get("duration", 0),
                    "view_count": existing_info.get("view_count", 0),
                    "like_count": existing_info.get("like_count", 0),
                },
            ...
            if subtitle_path:
                ...
                if subtitle_file_path.exists():
                    ...
                    # 更新项目处理配置中的字幕路径
                    cfg = dict(project.processing_config or {})
                    cfg["subtitle_path"] = str(new_subtitle_path)
                    project.processing_config = cfg
            ...
            try:
                ...
                # Limpieza del directorio temporal de la tarea.
                try:
                    import shutil
                    shutil.rmtree(download_dir, ignore_errors=True)
                except Exception as cleanup_error:
                    logger.warning(f"No se pudo limpiar carpeta temporal {download_dir}: {cleanup_error}")
        ...
        download_tasks[task_id].progress = 0.0
        download_tasks[task_id].updated_at = datetime.now().isoformat()
        await _mark_project_download_failed(project_id, str(e))
